...
__Notes:__ `bamCoverageScaleFactor` will use `CTRL` and `EXPERIMENT` input files to normalize the reads count of each samples, given the formula : 
...
* `4Cseq/4C-seq_process_trans.snakefile` : main script for snakemake, call all RULES and config file.
* `4Cseq/create_matrix_count.R` : script to get the 4C-seq reads count matrix for each bins. 
* `4Cseq/demultiplex.py` : demultiplex each viewpoint for each input file.
* `4Cseq/favorite_gaelle_smooth.R` : smooth bigwig.
* `4Cseq/make_frags.R` : create the bed file for MboI and NlaIII motif position.
* `4Cseq/normFrags.R` : normalize 4C-SEQ read counts.
* `4Cseq/smoothData_adapted.R` : smooth bedGraph.
* `4Cseq/EXCLUDES` : folder with position to exclude for each viewpoint.
...
* `DEMULTIPLEX_PATH` : path were the fastq fiels will be demultiplexed.
...
* `EXCLUDES_PATH` : folder location of position to exclude for each viewpoint.
...
* `INFO_FASTQ` : for each input file, specify :
    ...
    * `"viewpoint"` : the corresponding viewpoint from `PRIMERS` dictionary.
    * `"to.exclude"` : the corresponding region to exclude from `EXCLUDES` dictionary.
